...
function getAuthorization(req, res, next) {
  ...
  if (req.get('Authorization') == undefined) {
    clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, null, null, null, null, function (err, creq) {});
    ...
    if (authParam.length != 2) {
      clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, req.get('Authorization'), null, null, null, function (err, creq) {});
      next();
    }
    else {
      ...
      if (authParam[0] == 'Basic') {
        basicAuthentication(authParam[1], function (err, client) {
          ...
          if (err || client == null) {
            clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, req.get('Authorization'), 'basic', null, null, function (err, creq) {});
            ...
            clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, req.get('Authorization'), 'basic', client._id, null,
            function (err, creq) {
              if (err) {
        ...
        tokenAuthentication(authParam[1], function (err, token) {
          if (err || token == undefined) {
            clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, req.get('Authorization'), 'full', null, null, function (err, creq) {});
            if (err) {
              next(err);
            } else {
              next();
            ...
            clientRequestModel.createRequest(req.ip, req.method, req.url, req.query, req.body, req.get('Authorization'), 'full', token.client, token.user,
            function (err, creq) {
              if (err) {
                logger.error('Unable to create request');
                next(err);
